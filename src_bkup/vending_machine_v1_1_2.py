...
# ロジック関数（UIに依存しない計算）
# ==========================================
def process_drinks_buy(current_money: int, price: int):
    """
    商品購入処理
    購入判定とお釣り計算のみを行う
...
# 表示用関数（UIを描写する）
# ==========================================
def display_drink_menu(drinks: dict[str, int], row_size: int = 4):
    """
    商品一覧表示（多段表示）
...
if return_clicked:
    if st.session_state.total_money > 0:
        with return_money_msg.container():
            st.info(f"💰 お釣りは{st.session_state.total_money}円です。")
        st.session_state.total_money = 0
        time.sleep(2)
...
if buy_clicked:
    if st.session_state.total_money <= 0:
        side_msg.error("お金が投入されていません。")
    elif selected_item == "-- 選択してください --":
        side_msg.warning("商品を選択してください。")
    else:
        # 商品購入メソッド呼び出し
        price = DRINK_MENU[selected_item]
        success, change = process_drinks_buy(st.session_state.total_money, price)

        # 商品購入成功時の処理
        if success:
            st.session_state.total_money = change
            side_msg.info("ありがとうございました。")
            footer_msg.success(f"✅ {selected_item} を購入しました！", icon="🥤")
